 ...
 *
 * shared_arr[15:19][16:0] are the 4 archons' build queue max urgencies
 */
...
public class Comms {
	...
	public static void recv() throws GameActionException {
		...
		max_other_build_urgencies = 0;
		for (int i=4; --i>=0;) {
			archon_build_urgencies[i] = (i==archon_number)? 0 : shared_arr[15+i];
			max_other_build_urgencies = Math.max(max_other_build_urgencies, shared_arr[15+i]);
		}
	}
...
//					rc.setIndicatorLine(Info.loc, send_loc, 255, 0, 0);
		...
		if (Info.type == RobotType.ARCHON) {
			set(15+archon_number, Math.max(Math.max(Archon.builder_timer, Archon.miner_timer), Archon.soldier_timer));
		}
	}
