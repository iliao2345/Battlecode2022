...
//		if (Info.n_enemy_soldiers+Info.n_enemy_watchtowers+Info.n_enemy_sages > 0 && Info.type == RobotType.SOLDIER) {
		if (Info.enemy_robots.length > 0 && Info.type == RobotType.SOLDIER) {
			...
			for (int dx=2; --dx>=-1;) {
				for (int dy=2; --dy>=-1;) {
					values[dx+1][dy+1] = Math.max(max_value, values[dx+1][dy+1]);
				}
			}
//			double illegal_threshold = 1;
			double illegal_threshold = max_value;
			for (int dx=2; --dx>=-1;) {
				for (int dy=2; --dy>=-1;) {
	...
	public static double[][] battle_tile_values_dense() throws GameActionException {
		double enemy_rubble = rc.senseRubble(Info.enemy_robots[Info.rng.nextInt(Info.enemy_robots.length)].location);
		...
		for (Direction dir:Direction.allDirections()) {
			...
			if (rc.onTheMap(testloc)) {
				...
				values[dir.dx+1][dir.dy+1] = ((double)n_enemy)/(n_friendly + 1) * rc.senseRubble(testloc) / enemy_rubble;
				if (n_enemy == 0) {
					values[dir.dx+1][dir.dy+1] = 1;
				}
			}
			else {
				values[dir.dx+1][dir.dy+1] = Double.MIN_VALUE;
			}
		}
		return values;
	}
