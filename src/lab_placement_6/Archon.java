...
//	public static final int MIN_ARCHON_WARZONE_SEPARATION_DIST2 = (int)Math.pow(5, 2);
	...
	public static int last_builder_made_round = Integer.MIN_VALUE;
...
//		boolean send_initial_soldiers = Comms.total_miner_count >= Comms.n_starting_archons*3 && rc.getRobotCount()-rc.getArchonCount()-Comms.total_miner_count < Comms.n_starting_archons;
		...
		save_lead_for_transmutation = Comms.builder_exists && rc.getRobotCount()-rc.getArchonCount()-Comms.total_miner_count > 20*(Comms.laboratory_count+0.5) && Info.lead < 275;
		make_a_builder = !Comms.builder_exists && rc.getRobotCount()-rc.getArchonCount()-Comms.total_miner_count > 20*(Comms.laboratory_count+0.5);
		if (!make_a_builder) {
			builder_urgency = 0;
		}
...
//			miner_rate = Math.min(100, (int)(average_lead));
	...
	public static void try_to_move() throws GameActionException {
		if (enable_sacrifice) {
			...
			if (far_archon && health_to_heal==0 && nearby_miners && Info.n_enemy_miners==0 && rc.getArchonCount() > 1 && !initial_mining_phase) {
