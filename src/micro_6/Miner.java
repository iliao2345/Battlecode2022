...
//		if (gold_loc != null && rc.canSenseLocation(gold_loc) && rc.senseGold(gold_loc) == 0) {
//			gold_loc = null;
//		}
//		MapLocation[] visible_locations = rc.getAllLocationsWithinRadiusSquared(Info.loc, Info.VISION_DIST2);
//		MapLocation test_loc = visible_locations[Info.rng.nextInt(visible_locations.length)];
...
//			}
			...
			for (int dx = Math.min(1, Info.MAP_WIDTH-1-Info.x)+1; --dx >= Math.max(-1, -Info.x);) {
				for (int dy = Math.min(1, Info.MAP_HEIGHT-1-Info.y)+1; --dy >= Math.max(-1, -Info.y);) {
					...
					int test_lead = rc.senseLead(test_loc);
...
//	}
		...
		int most_lead = 0;  // get the location of the largest lead pile visible
