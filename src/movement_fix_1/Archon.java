...
//			controlled_tiles = controlled_tiles_checked/2;
		...
		boolean lacking_miners = Comms.total_miner_count < BEST_MINER_DENSITY * controlled_area && Info.n_friendly_miners < (rc.getRobotCount()-rc.getArchonCount())/2;
...
//			builder_rate = 0;
//		}
		else if (initial_mining_phase && !send_initial_soldiers) {
			soldier_rate = 0;
			builder_rate = 0;
			miner_rate = 100*self_closest_area/(Info.MAP_WIDTH*Info.MAP_HEIGHT);
		}
		else if (Info.round_num < stop_econ_round*1.3) {
			soldier_rate = 100;
			miner_rate = 0;
			builder_rate = 0;
		}
	...
	public static void try_to_move() throws GameActionException {
		...
		if (enable_sacrifice) {
			...
			for (int i=Info.n_friendly_miners; --i>=0;) {
				if (Info.loc.isWithinDistanceSquared(Info.friendly_miners[i].location, RobotType.MINER.visionRadiusSquared)) {
					nearby_miners = true;
					break;
				}
			}
...
//			Action.move(Bfs.way_to(best_loc, 100, true));
		...
		if (rc.getMode() == RobotMode.TURRET && !initial_mining_phase) {
			...
			if ((!best_loc.equals(Info.loc) && 10*(1+Info.rubble/10.) < 2*Comms.closest_attacker_dist*(1+Info.rubble_quantile/10.)*RobotType.SOLDIER.movementCooldown/10. || health_to_heal==0 && far_archon) && rc.isTransformReady()) {
				rc.transform();
				return;
			}
		}
	...
	public static void try_to_build() throws GameActionException {
		if (Info.gold >= 20 && Comms.most_urgent_archon && Info.action_ready) {
