...
//				Direction dir = Bfs.way_to(healing_loc, 100, true);
				...
				if (Comms.archon_healing_health[healing_archon_index] > 4*RobotType.SOLDIER.health + Info.health*3) {
					if (Info.health > 20) {
						healing = false;
...
//			Direction dir = Bfs.way_to(target_loc, 100, true);
		...
		else if ((closest_robot != null || Comms.attacker_seen_before && Comms.closest_attacker_dist < 11) && (!healing || Info.n_enemy_attackers > 0 && Info.loc.isWithinDistanceSquared(healing_loc, RobotType.ARCHON.actionRadiusSquared))) {
			...
			if (n_illegal_tiles == 8) {
				...
				MapLocation target_loc = closest_robot==null? Comms.closest_attacker_loc : closest_robot.location;
//				Pathing.target(closest_robot.location, illegal_tiles, 1);
				Pathing.approach(target_loc, illegal_tiles);
			}
			return;
		}
