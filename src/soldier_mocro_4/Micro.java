...
//				if ((enemy_shootable_now || outnumbered) && !chase_miners) {
	...
	public static double[][] battle_tile_values_sparser() throws GameActionException {
		...
		boolean[] enemy_soldiers_preoccupied = new boolean[Info.n_enemy_soldiers];
		boolean[] friendly_soldiers_preoccupied = new boolean[Info.n_friendly_soldiers];
		double friendly_hp = 0;
		double friendly_dps = 0;
		if (Info.n_friendly_soldiers < 5) {
			for (int i=Info.n_enemy_soldiers; --i>=0;) {
				...
				for (int j=Info.n_friendly_soldiers; --j>=0;) {
					...
					if (friend.location.isWithinDistanceSquared(enemy.location, 13)) {
						enemy_soldiers_preoccupied[i] = true;
						friendly_soldiers_preoccupied[j] = true;
					}
				}
			}

			for (int i=Info.n_friendly_soldiers; --i>=0;) {
				if (friendly_soldiers_preoccupied[i]) {
					rc.setIndicatorDot(Info.friendly_soldiers[i].location, 0, 255, 0);
					friendly_hp += Info.friendly_soldiers[i].health;
					friendly_dps += 30 / (10.+rc.senseRubble(Info.friendly_soldiers[i].location));
				}
				else {
					rc.setIndicatorDot(Info.friendly_soldiers[i].location, 255, 0, 0);
				}
			}
//			friendly_dps *= RobotType.SOLDIER.damage;
		}
		else {
			friendly_hp = Info.n_friendly_soldiers*RobotType.SOLDIER.health;
			friendly_dps = Info.n_friendly_soldiers*30 / (10.+Info.rubble_quantile);
		}

		double enemy_hp = 0;
		double enemy_dps = 0;
		for (int i=Info.n_enemy_soldiers; --i>=0;) {
			if (enemy_soldiers_preoccupied[i]) {
				rc.setIndicatorDot(Info.enemy_soldiers[i].location, 0, 255, 0);
				enemy_hp += Info.enemy_soldiers[i].health;
				enemy_dps += 30 / (10.+rc.senseRubble(Info.enemy_soldiers[i].location));
			}
			else {
				rc.setIndicatorDot(Info.enemy_soldiers[i].location, 255, 0, 0);
			}
		}
//		enemy_dps *= 10*RobotType.SOLDIER.damage;
...
//		}
		...
		for (Direction dir:Direction.allDirections()) {
			MapLocation testloc = Info.loc.add(dir);
			if (rc.onTheMap(testloc) && !rc.isLocationOccupied(testloc)) {
				double total_enemy_hp = enemy_hp;
				double total_enemy_dps = enemy_dps;
				boolean enemy_shootable = false;
				for (int i=Info.n_enemy_soldiers; --i>=0;) {
					if (testloc.isWithinDistanceSquared(Info.enemy_soldiers[i].location, RobotType.SOLDIER.actionRadiusSquared)) {
						if (!enemy_soldiers_preoccupied[i]) {
							total_enemy_hp += Info.enemy_soldiers[i].health;
							total_enemy_dps += 30 / (10.+rc.senseRubble(Info.enemy_soldiers[i].location));
						}
						enemy_shootable = true;
					}
				}
				double dist_component = Comms.attacker_seen_before? (enemy_shootable_now? 1 : -1) * (Comms.closest_attacker_dx*dir.dx+Comms.closest_attacker_dy*dir.dy)/Comms.closest_attacker_dist : 0;
//				double dist_component = dir.dx*dir.dx+dir.dy*dir.dy;
				double total_friendly_hp = friendly_hp + (enemy_shootable || enemy_shootable_now? Info.health : 0);
				double total_friendly_dps = friendly_dps + (enemy_shootable || enemy_shootable_now? 30 / (10.+rc.senseRubble(testloc)) : 0);
				if (Soldier.healing) {
					for (int i=Info.n_friendly_archons; --i>=0;) {
						if (Info.friendly_archons[i].location.isWithinDistanceSquared(Info.loc, RobotType.ARCHON.actionRadiusSquared)) {
							total_friendly_hp += RobotType.ARCHON.health;
						}
					}
				}
				values[dir.dx+1][dir.dy+1] = total_enemy_hp*total_enemy_dps-total_friendly_hp*total_friendly_dps + 0.001*dist_component;
			}
			else {
				values[dir.dx+1][dir.dy+1] = Double.MAX_VALUE;
			}
