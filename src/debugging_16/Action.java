...
public class Action {
	...
	public static void buildRobot(RobotType type, Direction dir) throws GameActionException {
		...
		if (type == RobotType.BUILDER) {
			...
			if (Archon.builder_rate + Archon.miner_rate + Archon.soldier_rate == 0) {
		...
		if (type == RobotType.MINER) {
			...
			for (int i=Math.min(20, Info.round_num+1); --i>=0;) {
				Archon.employment[i] += Archon.average_employment / Archon.n_average_employment;
				Archon.global_employment[i] += Archon.global_average_employment / Archon.n_global_average_employment;
