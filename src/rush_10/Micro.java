...
public class Micro {
	...
	public static void update() throws GameActionException {
		...
		if (Info.n_enemy_attackers > 0) {
			battle_ratio = (double)Info.n_friendly_attackers / Info.n_enemy_attackers;
		}
		else {
			battle_ratio = 1;
		}
	...
	public static double[][] battle_tile_values_dense() throws GameActionException {
		...
		for (Direction dir:Direction.allDirections()) {
			...
			if (rc.onTheMap(testloc)) {
				int n_enemy = rc.senseNearbyRobots(testloc, Info.VISION_DIST2, Info.enemy).length;
				int n_friendly = rc.senseNearbyRobots(testloc, Info.VISION_DIST2, Info.friendly).length;
				...
				if (n_friendly >= 5*n_enemy && dir.dx == 0 && dir.dy == 0) {
