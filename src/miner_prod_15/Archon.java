...
//	public static boolean initial_mining_phase = true;
	...
	public static double[] employment_rates = new double[20];
...
//		}
	...
	public static void post_comms_update() throws GameActionException {
		...
		double employment_rate = (Comms.archon_mining_miner_count+0.4/rc.getArchonCount())/(Comms.archon_miner_count+0.4/rc.getArchonCount());
		employment_rates[Info.round_num%20] = employment_rate;
		double average_employment_rate = 0;
		for (int i=Math.min(20, Info.round_num+1); --i>=0;) {
			average_employment_rate += employment_rates[i];
		}
		average_employment_rate /= Math.min(20, Info.round_num+1);
		lacking_miners = average_employment_rate > 0.7 || 5*lead_tiles.length + total_lead > 100*(Info.n_friendly_miners+1);
		rc.setIndicatorString(String.valueOf(average_employment_rate));
