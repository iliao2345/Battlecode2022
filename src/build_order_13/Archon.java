...
//		}
	...
	public static void post_comms_update() throws GameActionException {
		...
		double stop_econ_round = (Comms.total_miner_count > 3 + 3*Comms.archon_positions.length)? Info.round_num-1 : 2000;
		if (Info.round_num == 3) {
...
//			controlled_tiles = controlled_tiles_checked/2;
		...
		boolean lacking_miners = Comms.total_miner_count < BEST_MINER_DENSITY * controlled_area && Comms.total_miner_count < (rc.getRobotCount()-rc.getArchonCount())/2 || Comms.total_miner_count < 0.4*(rc.getRobotCount()-rc.getArchonCount()) || total_lead + 10*lead_tiles.length > 100*Info.n_friendly_miners;
...
//		save_lead_for_transmutation = Comms.builder_exists && Info.not_enough_laboratories && Info.lead < 275;
		save_lead_for_transmutation = false;
		make_a_builder = !Comms.builder_exists && Info.not_enough_laboratories;
		if (!make_a_builder) {
			builder_urgency = 0;
		}
		
		int n_sages = rc.getRobotCount()-rc.getArchonCount()-Comms.laboratory_count-Comms.total_miner_count-Comms.total_soldier_count;
		boolean enough_soldiers_in_army_mix = Math.pow(Comms.total_soldier_count, 2) > 0.04*n_sages*n_sages + 4;
//		boolean enough_soldiers_in_army_mix = true;
...
//			builder_rate = 0;
//		}
		else if (initial_mining_phase && !Info.not_enough_laboratories) {
...
//		}
			...
			miner_rate = 100*self_closest_area/(Info.MAP_WIDTH*Info.MAP_HEIGHT) + 30*total_lead/(Comms.total_miner_count+10);
			soldier_rate = 0;
			builder_rate = 0;
		}
		else if (!save_lead_for_transmutation && !enough_soldiers_in_army_mix && !Info.not_enough_laboratories) {
			miner_rate = 0;
...
//		rc.setIndicatorString(String.valueOf(stop_econ_round));
	...
	public static void try_to_move() throws GameActionException {
		if (enable_sacrifice) {
			...
			if (far_archon && health_to_heal==0 && nearby_miners && Info.n_enemy_miners==0 && rc.getArchonCount() > 1 && !initial_mining_phase && Comms.laboratory_count == 0) {
