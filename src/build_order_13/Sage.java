	...
	public static void post_comms_update() throws GameActionException {
		...
		for (int i=Comms.archon_positions.length; --i>=0;) {
			if (Comms.archons_alive[i]) {
				rc.setIndicatorLine(Info.loc, Comms.archon_positions[i], 0, 255, 0);
				...
				rc.setIndicatorLine(Info.loc, Comms.archon_positions[i], 255, 0, 0);
			}
		}
		if (healing_loc==null) {
			for (int i=Comms.archon_positions.length; --i>=0;) {
...
//			return;
//		}
//		if (rc.senseNearbyRobots(Info.ACTION_DIST2, Info.enemy).length >= 3 && (rc.senseNearbyRobots(13, Info.enemy).length > 0 || Info.n_enemy_soldiers < 3)) {
...
//		}
		...
		for (RobotInfo robot:enemy_robots) {
			damage += robot.type.isBuilding()? 0 : Math.min(robot.health, 0.22*robot.type.health);
			kills += (robot.type.isBuilding() || robot.health > 0.22*robot.type.health)? 0 : 1;
		}
		if (damage >= 70 || kills > 0) {
			Action.envision(AnomalyType.CHARGE);
			return;
		}
//		if (Info.n_enemy_attackers > 1) {
...
//		}
		...
		if (weakest_robot == null) {
			for (int i=Info.n_enemy_sages; --i>=0;) {
				if (Info.enemy_sages[i].health > strongest_health && Info.enemy_sages[i].health <= 45 && Info.loc.isWithinDistanceSquared(Info.enemy_sages[i].location, Info.ACTION_DIST2)) {
					weakest_robot = Info.enemy_sages[i];
					strongest_health = Info.enemy_sages[i].health;
				}
			}
		}
		if (weakest_robot == null) {
...
//		if (weakest_robot==null && Info.n_enemy_sages + Info.n_enemy_soldiers + Info.n_enemy_watchtowers + Info.n_enemy_laboratories > 0) {
//			return;
//		}
		if (weakest_robot == null) {
			for (int i=Info.n_enemy_miners; --i>=0;) {
		...
		if (weakest_robot != null) {
			if (weakest_robot.health < Math.floor(0.22*weakest_robot.type.health) && !weakest_robot.type.isBuilding()) {
				Action.envision(AnomalyType.CHARGE);
				return;
			}
			else if (45 <= Math.floor(0.1*weakest_robot.type.health) && Info.n_friendly_archons + Info.n_friendly_laboratories + Info.n_friendly_watchtowers == 0 && weakest_robot.type.isBuilding() && weakest_robot.mode == RobotMode.TURRET) {
...
//			return;
//		}
//		RobotInfo[] charge_affected_robots = rc.senseNearbyRobots(Info.ACTION_DIST2, Info.enemy);
//		if (charge_affected_robots.length > 7) {
//			Action.envision(AnomalyType.CHARGE);
//			return;
//		}
//		int charge_shots = 0;
//		for (RobotInfo robot:charge_affected_robots) {
//			charge_shots += robot.type.isBuilding()? 0 : (robot.health-1) / 45 - ((int)Math.max(0, robot.health-0.22*robot.type.health)-1) / 45;
//		}
//		
//		RobotInfo weakest_robot = null;
//		int weakest_health = Integer.MAX_VALUE;
...
//		}
//		if (weakest_robot != null) {
//			if (charge_shots > 1 || weakest_robot.health - 0.22*weakest_robot.type.health <= 0 && !weakest_robot.type.isBuilding()) {
//				Action.envision(AnomalyType.CHARGE);
//				return;
//			}
//			else if (45 <= Math.floor(0.1*weakest_robot.type.health) && Info.n_friendly_archons + Info.n_friendly_laboratories + Info.n_friendly_watchtowers == 0 && weakest_robot.type.isBuilding() && weakest_robot.mode == RobotMode.TURRET) {
...
//			Action.move(Bfs.direction(Bfs.query(target_loc, (Info.n_friendly_soldiers > 2*Info.n_enemy_soldiers + 4)? 100 : Math.min(100, Info.average_rubble + Math.max(0, Math.sqrt(Info.loc.distanceSquaredTo(target_loc))-4)*16), 2000, true)));
			Action.move(Bfs.direction(Bfs.query(target_loc, (Info.n_friendly_soldiers > 2*Info.n_enemy_soldiers + 4)? 100 : Info.rubble_quantile, 2000, true)));
			return;
		}
	}
