...
//	public static final double LEAD_FORCE_COEFFICIENT = -1/10.;
	public static double WARZONE_BUFFER = 6;
	...
	public static MapLocation closest_gold_loc = null;
	public static MapLocation closest_adjacent_gold_loc = null;
...
//			WARZONE_BUFFER = (Info.n_friendly_attackers > 0)? 0 : ((Info.health > 30)? 5 : 8);
			...
			for (MapLocation test_loc:lead_locs) {
				if (!Comms.attacker_seen_before || !test_loc.isWithinDistanceSquared(Comms.closest_attacker_loc, dist2_buffer)) {
			...
			closest_gold_loc = null;
			closest_adjacent_gold_loc = null;
			closest_dist2 = Integer.MAX_VALUE;
			int closest_adjacent_dist2 = Integer.MAX_VALUE;
			for (MapLocation test_loc:gold_locs) {
				if (Info.loc.isWithinDistanceSquared(test_loc, closest_dist2)) {
					if (Info.loc.isAdjacentTo(test_loc) && test_loc.isWithinDistanceSquared(Comms.closest_attacker_loc, closest_adjacent_dist2)) {
						closest_adjacent_gold_loc = test_loc;
						closest_adjacent_dist2 = test_loc.distanceSquaredTo(Comms.closest_attacker_loc);
					}
					if (!Comms.attacker_seen_before || !test_loc.isWithinDistanceSquared(Comms.closest_attacker_loc, dist2_buffer)) {
						closest_dist2 = Info.loc.distanceSquaredTo(test_loc);
						closest_gold_loc = test_loc;
					}
				}
			}
//		}
...
//		}
	...
	public static void try_to_mine()  throws GameActionException {
		...
		if (closest_adjacent_gold_loc != null) {
			while (rc.senseGold(closest_adjacent_gold_loc) > 0 && Info.action_ready) {
				Action.mineGold(closest_adjacent_gold_loc);
			}
		}
		while (Info.action_ready) {
			...
			int mine_until = (Info.n_enemy_attackers > Info.n_friendly_attackers && Info.n_enemy_archons == Info.n_friendly_archons || Info.n_enemy_archons > Info.n_friendly_archons)? 0 : 1;
//			int mine_until = (Info.n_enemy_attackers > Info.n_friendly_attackers || Info.n_enemy_archons > Info.n_friendly_archons)? 0 : 1;
//			int mine_until = (Info.n_enemy_attackers > Info.n_friendly_attackers)? 0 : 1;
	...
	public static void try_to_move() throws GameActionException {
		...
		if (closest_gold_loc != null) {  // collect gold
			MapLocation best_adjacent_loc = null;
			int best_rubble = Integer.MAX_VALUE;
			for (int dx=2; --dx>=-1;) {
				for (int dy=2; --dy>=-1;) {
					if (rc.canSenseLocation(closest_gold_loc.translate(dx, dy))) {
						int rubble = rc.senseRubble(closest_gold_loc.translate(dx, dy));
						if (rubble < best_rubble || rubble==best_rubble && !closest_gold_loc.translate(dx, dy).isWithinDistanceSquared(best_adjacent_loc, dx*dx+dy*dy)) {
							best_rubble = rubble;
							best_adjacent_loc = closest_gold_loc.translate(dx, dy);
						}
					}
				}
			}
...
//		if ((most_lead > 1 || most_lead_loc!=null) && !healing) {  // farm lead
			...
			for (int dx=2; --dx>=-1;) {
				for (int dy=2; --dy>=-1;) {
					if (rc.canSenseLocation(most_lead_loc.translate(dx, dy)) && (!Comms.attacker_seen_before || !Info.loc.translate(dx, dy).isWithinDistanceSquared(Comms.closest_attacker_loc, (int)(WARZONE_BUFFER*WARZONE_BUFFER)))) {
