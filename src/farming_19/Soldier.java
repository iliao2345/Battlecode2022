...
//		}
			...
			if (estimated_enemy_archon_starting_loc!=null) {
				...
				target_loc = new MapLocation(left_intersection.x + (Comms.first_soldier? 1 : 2) * (right_intersection.x - left_intersection.x)/3, left_intersection.y + (Comms.first_soldier? 1 : 2) * (right_intersection.y - left_intersection.y)/3);
				...
				rc.setIndicatorLine(Info.loc, archon_approx_loc, 255, 0, 255);
				rc.setIndicatorLine(archon_approx_loc.translate(-dy, dx), archon_approx_loc, 0, 255, 0);
				rc.setIndicatorLine(archon_approx_loc.translate(dy, -dx), archon_approx_loc, 0, 0, 255);
				rc.setIndicatorLine(Info.spawn_loc, left_intersection, 0, 255, 0);
				rc.setIndicatorLine(Info.spawn_loc, right_intersection, 0, 0, 255);
				rc.setIndicatorString(String.valueOf(Comms.first_soldier));
			}
			Action.move(Bfs.direction(Bfs.query(target_loc, Info.average_rubble, (Info.round_num-Info.last_move_turn)/10.+1, true)));
			return;
