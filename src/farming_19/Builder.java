...
public class Builder {
	...
	public static MapLocation farm_loc = null;
	
	public static void update() throws GameActionException {
		rc.setIndicatorString(String.valueOf(Info.income_i / Info.income_1));
		back_edge = true;
...
//			rc.setIndicatorLine(Info.loc, closest_robot.location, 255, 0, 0);
		...
		if (closest_robot != null) {
			...
			MapLocation repel_loc = Comms.enemy_seen_before? Comms.closest_enemy_loc : new MapLocation(Info.MAP_WIDTH/2, Info.MAP_HEIGHT/2);
			double best_score = Double.MAX_VALUE;
			for (int[] diff:REPAIR_OFFSETS) {
				MapLocation loc = closest_robot.location.translate(diff[0], diff[1]);
				double score = rc.canSenseLocation(loc)? rc.senseRubble(loc) - 0.0001*loc.distanceSquaredTo(repel_loc) : Double.POSITIVE_INFINITY;
				if (score < best_score) {
					best_score = score;
					best_loc = loc;
				}
...
//			if (Comms.enemy_seen_before && back_edge) {
...
//			rc.setIndicatorLine(Info.loc, target_loc, 128, 0, 0);
		...
		else if (Info.not_enough_laboratories) {
...
//			}
			...
			MapLocation best_loc = null;
			for (MapLocation loc:rc.getAllLocationsWithinRadiusSquared(Info.loc, 5)) {
...
//		}
	...
	public static void act() throws GameActionException {
		if (!Comms.is_farmer) {
			...
			if (rc.senseLead(Info.loc) == 0) {
				rc.disintegrate();
			}
			else if (farm_loc==null || Info.loc.equals(farm_loc)) {
				double angle = Info.rng.nextDouble()*2*Math.PI;
				farm_loc = Info.loc.translate((int)(1000*Math.cos(angle)), (int)(1000*Math.sin(angle)));
				farm_loc = Math2.extend_to_wall(Info.spawn_loc, farm_loc, Info.MAP_WIDTH, Info.MAP_HEIGHT);
			}
			Action.move(Bfs.direction(Bfs.query(farm_loc, 100, 2000, false)));
		}
	}
