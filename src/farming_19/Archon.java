...
//	public static final int MIN_ARCHON_WARZONE_SEPARATION_DIST2 = (int)Math.pow(5, 2);
	...
	public static double farmer_urgency = 0;
	...
	public static double farmer_rate = 0;
...
//	public static boolean initial_mining_phase = true;
	...
	public static boolean see_extra_lead = false;
	public static boolean send_initial_soldiers = false;
	public static boolean desired_farmer = false;
...
//		most_lead_loc = null;
		...
		see_extra_lead = total_lead + 5*lead_tiles.length > 100*(Info.n_friendly_miners + 2);
//		rc.setIndicatorString(String.valueOf(see_extra_lead));
	}
	...
	public static void post_comms_update() throws GameActionException {
		...
		if (Info.round_num >= 3) {
			...
			enemy_distance_estimate = Comms.enemy_seen_before? Comms.closest_enemy_dist : Math.sqrt(closest_possible_enemy_starting_archon_dist2);
		}
		...
		lacking_miners = summary_employment_rate > 0.7 || see_extra_lead;
...
//		make_a_builder = (!Comms.builder_exists || Comms.total_builder_count < 4 && Info.lead > 500) && Info.not_enough_laboratories && Info.round_num >= 3;
		make_a_builder = (!Comms.builder_exists || Comms.total_builder_count < 4 && Info.lead > 220 + 30*Comms.total_builder_count || Comms.total_builder_count < rc.getRobotCount() / 20 || Comms.total_builder_count < Comms.archon_mining_miner_count / 7) && Info.not_enough_laboratories && Info.round_num >= 3;
		if (!make_a_builder) {
			builder_urgency = 0;
		}
		...
		send_initial_soldiers = (Comms.enemy_seen_before || total_lead+Info.lead < RobotType.SOLDIER.buildCostLead * (2-Comms.total_soldier_count) + RobotType.MINER.buildCostLead) && Comms.enemy_seen_round > Info.round_num + 20 && Comms.total_soldier_count < 2;
//		rc.setIndicatorString(String.valueOf(send_initial_soldiers));

		boolean see_extra_miners = total_lead + 5*lead_tiles.length < 30*(Info.n_friendly_miners - 1) && Info.round_num > 100 && summary_employment_rate < 0.5;
...
//		rc.setIndicatorString(String.valueOf(lacking_miners));
		if (send_initial_soldiers) {
			miner_rate = 0;
			soldier_rate = 100;
			builder_rate = 0;
			farmer_rate = 0;
		}
		else if (lacking_miners) {
		...
		else if (see_extra_miners && Info.n_friendly_attackers > 3 && Comms.closest_enemy_dist > 10) {
			miner_rate = 0;
			soldier_rate = 0;
			builder_rate = 0;
			farmer_rate = 100*(1-employment_rate);
		}
		else {
	...
	public static void try_to_move() throws GameActionException {
		...
		if (rc.getMode() == RobotMode.PORTABLE) {
			...
			if (!see_extra_lead) {
				...
				int move_radius_squared = 13;
				MapLocation[] search_locs = rc.getAllLocationsWithinRadiusSquared(Info.loc, move_radius_squared);
				MapLocation target_loc = closest_lead_loc;
				for (MapLocation loc:search_locs) {
					if (rc.senseRubble(loc) + 0.001*Info.loc.distanceSquaredTo(loc) < lowest_rubble && loc.isWithinDistanceSquared(target_loc, Info.loc.distanceSquaredTo(target_loc)) && (!rc.isLocationOccupied(loc) || loc.equals(Info.loc))) {
						best_loc = loc;
						lowest_rubble = rc.senseRubble(loc) + 0.001*Info.loc.distanceSquaredTo(loc);
					}
				}
				rc.setIndicatorLine(Info.loc, best_loc, 255, 0, 0);
			}
			if (best_loc==null) {
				if (Comms.enemy_seen_before) {
			...
			if ((best_loc.equals(Info.loc) || Info.n_enemy_attackers > Info.n_friendly_attackers && rc.senseNearbyRobots(Info.loc, EMERGENCY_TURRET_DIST2, Info.enemy).length > 0) && Info.rubble <= Info.rubble_quantile && rc.canTransform()) {
				rc.transform();
				return;
			}
	...
	public static void try_to_build() throws GameActionException {
        ...
        if (closest_dir != null) {
        	if (desired_robot != RobotType.BUILDER || make_a_builder || desired_robot == RobotType.BUILDER && desired_farmer) {
        		if (desired_robot == RobotType.BUILDER && desired_farmer) {
        			Action.buildFarmer(closest_dir);
        			rc.setIndicatorLine(Info.loc, new MapLocation(0, Info.MAP_HEIGHT-1), 0, 255,0);
        		}
        		else {
                    Action.buildRobot(desired_robot, closest_dir);
        			rc.setIndicatorLine(Info.loc, new MapLocation(0, Info.MAP_HEIGHT-1), 255, 255,0);
        		}
        	}
            return;
        }
    ...
    public static void try_to_heal() throws GameActionException {
        ...
        if (Comms.attacker_seen_before && Comms.closest_attacker_dist < 10) {
        	...
        	for (RobotInfo robot:repairable_robots) {
        		if (robot.type == RobotType.SAGE && rc.canRepair(robot.location) && robot.health < weakest_health && robot.health < RobotType.SAGE.health) {
        			weakest_robot = robot;
        			weakest_health = robot.health;
        		}
        		else if (robot.type == RobotType.SOLDIER && rc.canRepair(robot.location) && 100+robot.health > weakest_health && robot.health < RobotType.SOLDIER.health) {
        			weakest_robot = robot;
        			weakest_health = 100+robot.health;
        		}
        		else if (robot.type == RobotType.MINER && rc.canRepair(robot.location) && (miners_inactive? 0 : 200) + robot.health > weakest_health && robot.health < RobotType.MINER.health) {
        			weakest_robot = robot;
        			weakest_health = (miners_inactive? 0 : 2000) + robot.health;
        		}
        	}
        	...
        	for (RobotInfo robot:repairable_robots) {
        		...
        		else if (robot.type == RobotType.SOLDIER && rc.canRepair(robot.location) && 100+robot.health > strongest_health && robot.health < RobotType.SOLDIER.health) {
        			strongest_robot = robot;
        			strongest_health = 100+robot.health;
        		}
        		else if (robot.type == RobotType.MINER && rc.canRepair(robot.location) && (miners_inactive? 200 : 0) + robot.health > strongest_health && robot.health < RobotType.MINER.health) {
        			strongest_robot = robot;
...
//        }
		if (miner_urgency > builder_urgency && miner_urgency > soldier_urgency && miner_urgency > farmer_urgency) {
			desired_robot = RobotType.MINER;
			communicated_urgency = (1<<11) + (int)miner_urgency;
//			rc.setIndicatorDot(Info.loc, 0, 255, 0);
        }
        if (builder_urgency > miner_urgency && builder_urgency > soldier_urgency && builder_urgency > farmer_urgency) {
			desired_robot = RobotType.BUILDER;
			desired_farmer = false;
//			communicated_urgency = (1<<12)+(int)builder_urgency;
			communicated_urgency = (int)enemy_distance_estimate;
//			rc.setIndicatorDot(Info.loc, 0, 0, 255);
        }
        if (soldier_urgency > builder_urgency && soldier_urgency > miner_urgency && soldier_urgency > farmer_urgency) {
			desired_robot = RobotType.SOLDIER;
			communicated_urgency = (1<<12) + (int)soldier_urgency;
//			communicated_urgency = Comms.enemy_seen_before? (1<<12) : (int)soldier_urgency;
//			rc.setIndicatorDot(Info.loc, 255, 0, 0);
        }
        if (farmer_urgency > builder_urgency && farmer_urgency > miner_urgency && farmer_urgency > soldier_urgency) {
			desired_robot = RobotType.BUILDER;
			desired_farmer = true;
			communicated_urgency = (int)enemy_distance_estimate;
//			communicated_urgency = Comms.enemy_seen_before? (1<<12) : (int)soldier_urgency;
...
//		rc.setIndicatorString(String.valueOf(communicated_urgency));
	}
