 ...
 * 
 * shared_arr[20][0] is set to 1 if at least one archon is lacking_miners, else 0
 * 
 ...
 * 
 * shared_arr[46:54] are ids of builders marked for suicide
 */
...
public class Comms {
	...
	public static boolean any_archon_needs_lead = false;
	public static boolean first_soldier = true;
	public static boolean determined_suicide = false;
...
//			}
		...
		if (Info.type == RobotType.ARCHON && (Info.round_num >= 2 && Info.round_num <= 3) || Info.type == RobotType.SOLDIER && possible_enemy_archon_locs.length == 0) {  // enemy archon position evaluation
		...
		if (Info.type == RobotType.ARCHON) {  // count miners and track builder presence and count labs
			...
			if (first_archon) {
				...
				set(20, (Archon.lacking_miners || Archon.send_initial_soldiers)? 1 : 0);
			}
			else {
				set(20, Archon.lacking_miners? 1 : shared_arr[20]);
			}
			if (last_archon) {
		...
		if (Info.type == RobotType.LABORATORY) {
			any_archon_needs_lead = shared_arr[20]==1;
		}
		...
		if (Info.type == RobotType.SOLDIER) {
			first_soldier = shared_arr[36+Info.round_num%2] == 0;
...
//		closest_attacker_dy = Math.min(Math.max(closest_attacker_dy, -Info.y), Info.MAP_HEIGHT-Info.y-1);
		...
		if (Info.type == RobotType.BUILDER && !determined_suicide) {
			determined_suicide = true;
			is_farmer = rc.readSharedArray(46)%2 == 1 && rc.readSharedArray(46)>>1 > Info.round_num - 3;
		}
	}
