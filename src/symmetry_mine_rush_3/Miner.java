...
//	public static MapLocation gold_loc = null;
	...
	public static boolean seen_enemy_miner = false;
	public static boolean rush_mode = true;
	public static boolean role_determined = false;
...
//		if (Info.round_num < 20) {
			...
			for (MapLocation test_loc:lead_locs) {
				total_lead += rc.senseLead(test_loc);
...
//		}
		...
		seen_enemy_miner = seen_enemy_miner || Info.n_enemy_miners > 0;
...
//				Action.move(Bfs.way_to(most_lead_loc, Info.average_rubble, Clock.getBytecodesLeft() > 3500));
		...
		if (Info.move_ready) {  // explore
			if (!Info.encountered_enemies && rush_mode) {
				MapLocation target_loc = new MapLocation(Info.MAP_WIDTH/2, Info.MAP_HEIGHT/2);
				if (Comms.target_upon_spawn != null) {
					target_loc = Comms.target_upon_spawn;
				}
				Action.move(Bfs.way_to(target_loc, Info.average_rubble, false));
				return true;
			}
...
//			if (!Info.encountered_attackers) {
			if (!Comms.attacker_seen_before || Comms.closest_attacker_dist > (1+Info.average_rubble/10.)*Comms.attacker_seen_round - Info.round_num) {
//			if (true) {
