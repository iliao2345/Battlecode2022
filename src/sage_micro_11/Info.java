...
//		not_enough_laboratories = rc.getRobotCount()-rc.getArchonCount() > 10*Comms.laboratory_count + 2*Comms.archon_positions.length;
		not_enough_laboratories = rc.getRobotCount()-rc.getArchonCount() > 10*Comms.laboratory_count && Comms.laboratory_count > 0 && Info.income_i / Info.income_1 > 4 * Math.min(2, Comms.laboratory_count+1);
		
		//  This must be last
		Bfs.reset();
		if (rc.getType()==RobotType.ARCHON) {
			Archon.update();
//			not_enough_laboratories = not_enough_laboratories || Comms.laboratory_count==0 && Comms.builder_exists || Archon.total_lead < 180/Comms.archon_positions.length && !Comms.builder_exists;
			...
			rc.setIndicatorString(String.valueOf(all_guaranteed_lead));
			not_enough_laboratories = not_enough_laboratories || Comms.laboratory_count==0 && Comms.builder_exists || all_guaranteed_lead < 220 && !Comms.builder_exists && Info.round_num >= 3;
		}
		else if (type==RobotType.BUILDER) {
			not_enough_laboratories = not_enough_laboratories || Comms.laboratory_count==0 && round_num < 100;
			Builder.update();
		}
