...
public class Math2 {
	...
	public static MapLocation extend_to_wall(MapLocation from, MapLocation to, int W, int H) {
		if (to.equals(from)) {
			return new MapLocation(W, to.y);
		}
		double mult = Double.POSITIVE_INFINITY;
		if (to.x > from.x) {
			mult = Math.min(mult, (W-1.-from.x)/(double)(to.x-from.x));
		}
		else if (to.x < Info.x) {
			mult = Math.min(mult, (0.-from.x)/(double)(to.x-from.x));
		}
		if (to.y > from.y) {
			mult = Math.min(mult, (H-1.-from.y)/(double)(to.y-from.y));
		}
		else if (to.y < from.y) {
			mult = Math.min(mult, (0.-from.y)/(double)(to.y-from.y));
		}
		double bound_x = from.x + mult*(to.x-from.x);
		double bound_y = from.y + mult*(to.y-from.y);
		return new MapLocation(Math.max(0, Math.min(W-1, (int)bound_x)), Math.max(0, Math.min(H-1, (int)bound_y)));
	}
}