...
//				Direction dir = Bfs.way_to(healing_loc, 100, true);
		...
		if (gold_locs.length > 0) {  // collect gold
			if (!Info.loc.isAdjacentTo(gold_locs[0])) {
				Pathing.approach(gold_locs[0], new boolean[3][3]);
			}
			else {
				return Pathing.touch(gold_locs[0], new boolean[3][3]);
			}
			return true;
		}
...
//				Action.move(Bfs.way_to(lead_loc, Info.rubble_quantile, false));
				Action.move(Bfs.direction(Bfs.query(lead_loc, Info.rubble_quantile, 2000, true)));
				return true;
			}
//			else {
...
//				Action.move(Bfs.way_to(explore_loc, 100, false));
	...
	public static void get_new_explore_loc() throws GameActionException {
		...
		if (explore_loc == null) {
			for (int i=Comms.archon_positions.length; --i>=0;) {
				if (Comms.archons_alive[i] && Info.loc.isWithinDistanceSquared(test_loc, test_loc.distanceSquaredTo(Comms.archon_positions[i])) && !Comms.archon_positions[i].isWithinDistanceSquared(Info.spawn_loc, 20)) {
					closest_to_me = false;
				}
			}
		}
