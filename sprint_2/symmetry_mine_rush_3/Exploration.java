...
public class Exploration {
	...
	public static void update() throws GameActionException {
		...
		archon_midpoints = new MapLocation[Comms.archon_positions.length*Comms.archon_positions.length*((Comms.x_symmetry_possible?1:0) + (Comms.x_symmetry_possible?1:0) + (Comms.x_symmetry_possible?1:0))];
		int counter = 0;
		for (int i=Comms.archon_positions.length; --i>=0;) {
			for (int j=Comms.archon_positions.length; --j>=0;) {
				if (Comms.x_symmetry_possible) {
					archon_midpoints[counter] = new MapLocation(Info.MAP_WIDTH/2, Comms.archon_positions[i].y);
					counter++;
				}
				if (Comms.y_symmetry_possible) {
					archon_midpoints[counter] = new MapLocation(Comms.archon_positions[i].x, Info.MAP_HEIGHT/2);
					counter++;
				}
				if (Comms.xy_symmetry_possible) {
					archon_midpoints[counter] = new MapLocation(Info.MAP_WIDTH/2, Info.MAP_HEIGHT/2);
					counter++;
				}
			}
		}
