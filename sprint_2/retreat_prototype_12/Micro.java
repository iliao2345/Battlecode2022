...
//		}
		...
		chase_enemy_econ_units = Info.n_enemy_attackers == 0;
...
//		max_enemy_seen = (Info.n_friendly_attackers+1 >= max_enemy_seen)? 0 : max_enemy_seen;
		retreat_timer = (Info.n_enemy_attackers > Info.n_friendly_attackers+1)? 2 : Math.max(0, retreat_timer-1);
...
//		if (Info.n_enemy_soldiers+Info.n_enemy_watchtowers+Info.n_enemy_sages > 0 && Info.type == RobotType.SOLDIER) {
		...
		if (Info.enemy_robots.length > 0) {
			...
			double min_value = Math.min(values[0][0], Math.min(values[0][1], Math.min(values[0][2], Math.min(values[1][0], Math.min(values[1][1], Math.min(values[1][2], Math.min(values[2][0], Math.min(values[2][1], values[2][2]))))))));
//			double illegal_threshold = 1;
			double illegal_threshold = min_value;
			illegal_tiles[0][0] = values[0][0] > illegal_threshold;
			illegal_tiles[0][1] = values[0][1] > illegal_threshold;
			illegal_tiles[0][2] = values[0][2] > illegal_threshold;
			illegal_tiles[1][0] = values[1][0] > illegal_threshold;
			illegal_tiles[1][1] = values[1][1] > illegal_threshold;
			illegal_tiles[1][2] = values[1][2] > illegal_threshold;
			illegal_tiles[2][0] = values[2][0] > illegal_threshold;
			illegal_tiles[2][1] = values[2][1] > illegal_threshold;
			illegal_tiles[2][2] = values[2][2] > illegal_threshold;
	...
	public static double[][] battle_tile_values_sparser() throws GameActionException {
		...
		for (Direction dir:Direction.allDirections()) {
			...
			if (rc.onTheMap(testloc) && !rc.isLocationOccupied(testloc)) {
				...
				int n_enemy_visible = rc.senseNearbyRobots(testloc, Info.VISION_DIST2, Info.enemy).length;
				for (int i=Info.n_enemy_soldiers; --i>=0;) {
					if (testloc.isWithinDistanceSquared(Info.enemy_soldiers[i].location, 34)) {
						n_enemy_visible++;
						if (testloc.isWithinDistanceSquared(Info.enemy_soldiers[i].location, 20)) {
							n_enemy_shootable++;
						}
					}
				}
				for (int i=Info.n_enemy_watchtowers; --i>=0;) {
					if (testloc.isWithinDistanceSquared(Info.enemy_watchtowers[i].location, 34)) {
						n_enemy_visible++;
						if (testloc.isWithinDistanceSquared(Info.enemy_watchtowers[i].location, 20)) {
							n_enemy_shootable++;
						}
					}
				}
				for (int i=Info.n_enemy_sages; --i>=0;) {
					if (testloc.isWithinDistanceSquared(Info.enemy_sages[i].location, 34)) {
						n_enemy_visible++;
						if (testloc.isWithinDistanceSquared(Info.enemy_sages[i].location, 20)) {
							n_enemy_shootable++;
						}
					}
				}
				if (enemy_shootable_now) {
					values[dir.dx+1][dir.dy+1] = +1000*n_enemy_shootable+n_enemy_visible + 2000*(Math.floor((10+rc.senseRubble(testloc)/10.)*RobotType.SOLDIER.actionCooldown)) + (Comms.attacker_seen_before? -0.001*Math.hypot(Comms.closest_attacker_dx-dir.dx, Comms.closest_attacker_dy-dir.dy) : 0);
				}
				else {
					double neg_approx_distance = 0;
					if (Comms.attacker_seen_before) {
