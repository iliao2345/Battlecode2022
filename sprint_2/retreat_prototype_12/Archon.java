...
//			miner_rate = Math.min(100, (int)(average_lead));
	...
	public static void try_to_move() throws GameActionException {
		if (rc.getMode() == RobotMode.PORTABLE) {
			...
			double query = Bfs.query(best_loc, 100, 10, true);
			Direction dir = Bfs.direction(query);
//			rc.setIndicatorLine(Info.loc, best_loc, 0, 255, 0);
			rc.setIndicatorLine(Info.loc, rc.adjacentLocation(dir), 0, 255, 255);
			if (Info.move_ready) {
				Action.move(dir);
			}
//			Action.move(Bfs.direction(Bfs.query(best_loc, 100, 10, true)));
			return;
		}
		if (rc.getMode() == RobotMode.TURRET && !initial_mining_phase) {
			...
			for (AnomalyScheduleEntry anomaly:rc.getAnomalySchedule()) {
				if (anomaly.anomalyType == AnomalyType.VORTEX && anomaly.roundNumber == Info.round_num-1) {
					rc.transform();
					return;
				}
			}
