...
//		}
//		else {
//			low_hp = Info.health <= (healing_loc.isWithinDistanceSquared(Info.loc, 144)? 14 : 8) + 3*(1+Info.rubble/10.)*Math.max(1, Info.n_enemy_attackers)*Math.max(1, (Math.sqrt(RobotType.SOLDIER.actionRadiusSquared)-Comms.closest_attacker_dist));
//		}
			low_hp = Comms.attacker_seen_before && Info.health <= 5 + 3*(1+Info.rubble/10.)*Math.max(1, Info.n_enemy_attackers)*Math.max(1, (Math.sqrt(RobotType.SOLDIER.actionRadiusSquared)-Comms.closest_attacker_dist));
...
//				Direction dir = Bfs.way_to(healing_loc, 100, true);
				Direction dir = Bfs.direction(Bfs.query(healing_loc, 100, 10, true));
				Action.move(dir);
				return;
			}
		}
		...
		else if ((Micro.chase_enemy_econ_units && Comms.closest_attacker_dist > 7 || Info.n_enemy_archons>0 && Info.n_friendly_attackers > Info.n_enemy_attackers) && closest_robot != null) {
...
//			Action.move(Bfs.way_to(closest_robot.location, 100, true));
			Action.move(Bfs.direction(Bfs.query(closest_robot.location, 100, 10, true)));
			return;
		}
		else if (Micro.outnumbered && !healing) {
...
//			Direction dir = Bfs.way_to(target_loc, 100, true);
			Direction dir = Bfs.direction(Bfs.query(target_loc, 100, 10, true));
			Action.move(dir);
			return;
		}
...
//			Action.move(Bfs.way_to(Comms.closest_attacker_loc, Info.average_rubble, Clock.getBytecodesLeft()>2500));
			Action.move(Bfs.direction(Bfs.query(Comms.closest_attacker_loc, Info.average_rubble, 10, Clock.getBytecodesLeft()>2500)));
...
//			Action.move(Bfs.way_to(Comms.closest_enemy_loc, Info.average_rubble, Clock.getBytecodesLeft()>2500));
			Action.move(Bfs.direction(Bfs.query(Comms.closest_enemy_loc, Info.average_rubble, 10, Clock.getBytecodesLeft()>2500)));
//			boolean[][] illegal_tiles = new boolean[3][3];
...
//			Action.move(Bfs.way_to(target_loc, Info.average_rubble, true));
			Action.move(Bfs.direction(Bfs.query(target_loc, Info.average_rubble, 10, true)));
			return;
		}
		return;
	}
