package dynamic_build_order_14;
...
//		}
//		else {
			low_hp = Info.health <= (healing_loc.isWithinDistanceSquared(Info.loc, 144)? 11 : 5) + 3*(1+Info.rubble/10.)*Math.max(1, Info.n_enemy_attackers)*Math.max(1, (Math.sqrt(RobotType.SOLDIER.actionRadiusSquared)-Comms.closest_attacker_dist));
//		}
...
//				Direction dir = Bfs.way_to(healing_loc, 100, true);
		...
		else if ((Micro.chase_enemy_econ_units && Comms.closest_attacker_dist > 7 && Info.n_enemy_attackers==0 || Info.n_enemy_archons>0 && Info.n_friendly_attackers > Info.n_enemy_attackers) && closest_robot != null || Micro.winning_1v1_soldier) {
